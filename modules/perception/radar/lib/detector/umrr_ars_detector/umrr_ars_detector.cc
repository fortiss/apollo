 ...
 *****************************************************************************/
#include "modules/perception/radar/lib/detector/umrr_ars_detector/umrr_ars_detector.h"
...
bool UmrrArsDetector::Init() { return true; }

bool UmrrArsDetector::Detect(const drivers::UmrrRadar& corrected_obstacles,
...
std::string UmrrArsDetector::Name() const { return "UmrrArsDetector"; }

void UmrrArsDetector::RawObs2Frame(
  ...
  for (const auto radar_obs : corrected_obstacles.umrrobs()) {
    ...
    radar_object->velocity_uncertainty =
        (radar2world_rotate * vel_rms * vel_rms.transpose() *
         radar2world_rotate_t)
            .cast<float>();
    double local_obj_theta = 0.0; // EW radar_obs.oritation_angle() / 180.0 * PI;
    ...
    ADEBUG << "obs_id: " << radar_obs.obstacle_id() << ", "
           ...
           // EW << "rcs: " << radar_obs.rcs() << ", "
           // EW << "meas_state: " << radar_obs.meas_state();
           ;
  }
}

PERCEPTION_REGISTER_DETECTOR(UmrrArsDetector);
