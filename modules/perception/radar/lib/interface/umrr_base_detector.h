...
class UmrrBaseDetector {
 public:
  UmrrBaseDetector() = default;
  virtual ~UmrrBaseDetector() = default;
  ...
  virtual bool Detect(const drivers::UmrrRadar& corrected_obstacles,
 ...
 private:
  DISALLOW_COPY_AND_ASSIGN(UmrrBaseDetector);
};

PERCEPTION_REGISTER_REGISTERER(UmrrBaseDetector);
#define PERCEPTION_REGISTER_DETECTOR(name) \
  PERCEPTION_REGISTER_CLASS(UmrrBaseDetector, name)
