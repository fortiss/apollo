...
class RTCnet(nn.Module):
    ...
    def __init__(self, 
        ...
        # EW added
        self.FCN_nodes = [self.high_dims, 128, 128, num_classes]
    ...
    def forward(self, features):
        ...
        # EW comments
        # EW for i in range(len(self.conv_modules_para_2D)):
        # EW    low_feat = self.conv_modules_para_2D[i](low_feat)
        # EW low_feat = low_feat.view(-1,  self.channels_para_2D[-1], self.Doppler_dims)

        # EW comments
        # EW for i in range(len(self.conv_modules_1D)):
        # EW    low_feat = self.conv_modules_1D[i](low_feat)
        # EW low_feat = low_feat.view(-1, self.channels_1D[-1]*4, 1).contiguous()
