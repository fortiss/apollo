...
from apollo.drivers.radar.umrr_driver.umrr_can_publisher_pb2 import RadarConf
from apollo.drivers.radar.umrr_driver.umrr_can_publisher_pb2 import CanConf
from apollo.drivers.radar.umrr_driver.umrr_can_publisher_pb2 import ModeSetup
...
class com_sensor:
    ...
    def __init__(self, rc, cc):
        ...
        if hasattr(self.radar_conf, 'can_socket') and not(self.radar_conf.can_socket is None) and hasattr(self.can_conf, "can_spec") and not(self.radar_conf.can_spec is None):
            # CRT
            self.sock = radar_config.can_socket
            self.spec = can_conf.can_spec
...
class crt_umrr_publisher:
    ...
    def __create_serv(self):
        ...
        self.ummrr_node.create_service(self, 'sensor_status', sens_status, sens_status, lambda msg: self.__status_req(msg, self.con_sensor), args=None)
        self.ummrr_node.create_service(self, 'sensor_status', sens_status, sens_status, lambda msg: self.__status_req(msg, self.con_sensor), args=None)
    ...
    @staticmethod
    def __create_pcl_msg(list_of_targets, seq, fields, field_names):
        """
        Function fills the template with data coming from the sensor
        """
        pointcloud_targets = []
        # if thats the case warn about this error
        #CRT
        if self.radar_conf.debug:
            # gettin targets from the list of targets - these are identified objects
            for target in list_of_targets:
                pointcloud_target = [None] * len(fields)
                for field in fields:
                    # check for fields that must be present in pointcloud
                    if field.name == "Elevation" or field.name == "Azimuth" or field.name == "Range":
                        try:
                        ...
                        # optional field names, if not present just return 0
                        pointcloud_target[field_names.index(field.name)] = target.get(field.name, 0)
                else:
                    # append target to pointcloud
            ...
            for target in list_of_targets:
                ...
                for field in fields:
                    ...
                    try:
                        ...
                        # CRT
                        logging.warning("Incomplete target data received. Skipping result.")
                        break
                else:
        ...
        header = Header()
        # CRT
        header.stamp = cyber.Time.now()
        # CRT
        header.frame_id = self.radar_conf.frame_id
        header.seq = seq

        # could be: pc2.create_cloud(header,height,width,fields,pointcloud_targets)
